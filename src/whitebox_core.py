...
class WhiteBoxCore(WhiteBoxCommands):
	"""
		Класс корня проекта, наследует события из класса WhiteBoxCommands
	"""
	...
	def init_menu(self):
		"""
			инициализация меню главного окна
		"""
		# создание меню
	...
	def bind_commands(self):
		"""
			связывание окон и фреймов с командами
		"""
		# привязка событий к фрэйму визуализации иерархии
		tree = self.main_win.hierarchy_frame.tree_widget
		tree.bind("<Double-Button-1>", self.hierarchy_click)
		# привязка событий к фрэйму визуализации членов файла
		tree = self.main_win.file_info_frame.info_tree
		tree.bind("<Double-Button-1>", self.file_info_click)
		# привязка событий к фрэйму визуализации содержимого файлов
